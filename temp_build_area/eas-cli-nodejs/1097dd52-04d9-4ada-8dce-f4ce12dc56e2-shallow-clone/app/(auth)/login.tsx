            ...
            >
              <View style={styles.formCard}>
                ...
                <TouchableOpacity
                  onPress={async () => {
                    try {
                      // Dynamically import to avoid errors if module is not linked
                      const { googleAuthService } = await import('../../services/googleAuth');
                      await googleAuthService.signIn(); // No params = Login intent (no auto-register)

                      // Success handled in service (tokens set)
                      // Navigate to home
                      router.replace('/(tabs)');
                    } catch (err: any) {
                      if (err.message !== 'CANCELLED') {
                        ...
                        if (err.response?.status === 404 || errorMessage.includes('USER_NOT_FOUND')) {
                          showAlert(
                            'Kayıt Bulunamadı',
                            'Bu Google hesabı ile kayıtlı bir kullanıcı bulunamadı. Lütfen önce kayıt olun.',
                            'info',
                            [
                              { text: 'İptal', style: 'cancel', onPress: () => setAlertConfig(prev => ({ ...prev, visible: false })) },
                              {
                                text: 'Kayıt Ol',
                                onPress: () => {
                          ...
                          showAlert('Hata', errorMessage || 'Google girişi başarısız', 'error');
                        }
                      }
                    }
                  }}
                  activeOpacity={0.8}
                  style={[styles.loginButtonWrapper, { marginTop: 0, marginBottom: spacing.lg, backgroundColor: 'white' }]}
                >
                  <View style={styles.loginButtonGradient}>
                    <Ionicons name="logo-google" size={20} color={colors.text} />
                    <Text style={[styles.loginButtonText, { color: colors.text }]}>Google ile Devam Et</Text>
                  </View>
                </TouchableOpacity>
