...
export const authenticate = async (
  ...
  try {
    ...
    try {
      ...
      // Database connection error - but token is valid, allow with decoded user info
      // This is a fallback for development when database is down
      if (isConnectionError || decoded.id.startsWith('mock-')) {
        console.warn('⚠️ Database connection failed, using token user info as fallback');
        req.user = {
          id: decoded.id,
          email: decoded.email,
          userType: decoded.userType,
        ...
        next();
      } else if (dbError instanceof UnauthorizedError && decoded.id.startsWith('mock-')) {
        // Mock user için UnauthorizedError'ı ignore et
        console.warn('⚠️ Mock user detected, allowing access');
        req.user = {
          id: decoded.id,
          email: decoded.email,
          userType: decoded.userType,
        ...
        // Other database errors (user not found, etc.)
        next(dbError);
      }
    }
  } catch (error) {
